    ...
    async jwt({ token, user, account }) {
      ...
      if (account && user) {
        try {
          ...
          const backendResponse = await signInWithGoogle(payload);

          // --- 3. 백엔드 응답 데이터를 NextAuth JWT 토큰에 저장 ---
          token.accessToken = backendResponse.data.accessToken; // ⭐️ 백엔드의 Access Token 저장
          token.userId = backendResponse.data.user.id; // ⭐️ 백엔드의 사용자 ID 저장
